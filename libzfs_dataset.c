...
make_dataset_handle_common(zfs_handle_t* zhp, zfs_cmd_t* zc)
{
    ...
    /*
     * Проверяем, есть ли активные удержания (holds) перед удалением снапшота.
     */
    if (errno == EBUSY) {
	nvlist_t* holds = NULL;
	err = lzc_get_holds(zhp->zfs_name, &holds);

	if (err == 0 && holds != NULL && nvlist_next_nvpair(holds, NULL) != NULL) {
	    nvlist_free(holds);
	    fprintf(stderr, "cannot destroy '%s': snapshot has active holds\n", zhp->zfs_name);
	    return (EZFS_ACTIVE_POOL);  // Используем EZFS_ACTIVE_POOL вместо EZFS_ACTIVE_HOLDS
	}
	nvlist_free(holds);
    }

    /*
     * Устанавливаем тип объекта ZFS.
     */
